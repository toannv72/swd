...
import { Button, Input, InputNumber, Modal, Space, Table, Tooltip, Typography, notification } from 'antd';
...
export default function OrderDeposit({ activeTab }) {
    ...
    const processing = () => {
        putData('customOrder/admin/put/price', orderRequestDefault.id, { price:price })
            .then((e) => {
                setDataRun(!dataRun);
            })
    ...
    const processingS = () => {
        putData('customOrder/admin/put/price', 'Processing', { orders: selected,price:price })
            .then((e) => {
                setDataRun(!dataRun);
            })
            .catch(err => console.log(err))
    ...
    return (
        <>
            ...
            <Modal title="Chấp nhận đơn hàng này"
                ...
                <div className='text-lg p-6'>Bạn có chắc chắn muốn xác nhận đặt cọc hay không?</div>

               <div className='flex gap-2 justify-center p-2 flex-col'>
                    Số tiền cọc đã cọc
                    <InputNumber
                     style={{ width: 200 }}
                        defaultValue={price}
